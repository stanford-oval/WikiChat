...
def load_templates(file, output_file=None):
    ...
    for line in file:
        ...
        if tag == "page":
            ...
            if output_file and (
                title.startswith(Extractor.templatePrefix)
                or title.startswith(modulePrefix)
            ):
                output.write("<page>\n")
...
def collect_pages(text):
    ...
    for line in text:
        ...
        if tag == "page":
            ...
            if (
                (
                    ...
                    not in [
                        ...
                        "File",
                        "Wikipedia",
                        "Help",  # e.g. https://en.wikipedia.org/wiki/Help:Table
                        "Template",  # e.g. https://en.wikipedia.org/wiki/Template:Date
                        "Portal",  # e.g. https://en.wikipedia.org/wiki/Portal%3ABiography
                        "Category",  # e.g. https://en.wikipedia.org/wiki/Category:Singers
                        "MediaWiki",
                        "Draft",
                        "Module",
                        "TimedText",
                    ...
                    or title[:colon] in acceptedNamespaces
                )
                and id != last_id
                and not redirect
                and not title.startswith(templateNamespace)
            ):
                # logging.info("accepting title %s", title)
                total_accepted += 1
                yield (id, revid, title, page)
                last_id = id
            else:
                # logging.info("rejecting title %s", title)
                total_rejected += 1
            id = ""
            revid = ""
            page = []
            inText = False
            redirect = False
        # if total_accepted > 10000:
        # break
    logging.info("total accepted articles: %d", total_accepted)
    logging.info("total rejected articles: %d", total_rejected)
...
def process_dump(
    ...
    for _ in range(max(1, process_count)):
        extractor = Process(
            target=extract_process, args=(jobs_queue, output_queue, html_safe)
        )
...
def main():
    ...
    groupP.add_argument(
        "--no-templates", action="store_true", help="Do not expand templates"
    )
    groupP.add_argument(
        "--html-safe",
        default=True,
